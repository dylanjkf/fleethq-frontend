/**
 * Mirrors apps/api/src/common/permissions/permission-catalog.ts exactly.
 * Duplicated rather than imported because the frontend and backend are
 * separate deployables (apps/fleethq, apps/api) with no shared package today
 * — see the Decision Log entry for this build for why a shared-types package
 * wasn't introduced yet (Tier 1: revisit if drift between the two actually
 * happens; the type-level PermissionKey union below still catches typos even
 * without a shared source file). If you add a permission on the backend, add
 * it here too.
 */
...
/** Same categorisation as the backend catalog — drives the role editor's grouped checkboxes. */
