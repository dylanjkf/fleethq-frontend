...
export async function enablePushNotifications(): Promise<void> {
  ...
  if (!isSecureForPush()) {
    throw new Error('Browser notifications need a secure connection — open the app over https, or on localhost during development.');
  }

  const permission = await Notification.requestPermission();
  if (permission === 'denied') throw new Error('Notifications are blocked for this site. Allow them in your browser’s site settings, then try again.');
  if (permission !== 'granted') throw new Error('Notification permission was not granted.');

  const publicKey = await getVapidPublicKey();
  if (!publicKey) throw new Error('Browser notifications aren’t configured on the server. Set VAPID keys in the API .env (npx web-push generate-vapid-keys) and restart it.');
