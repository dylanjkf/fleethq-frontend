 ...
 * public/sw.js) is populated the first time the user opens FleetHQ
 ...
 * enabled push, so a user who never turned push on had no offline shell
 * at all.
 *
 ...
 * That's expected; see 07-FleetHQ/FleetHQ_Overview.md's run notes.
 *
 ...
 * module URLs (e.g. `/src/features/fleet/FleetPage.tsx`) that change content
 * without changing URL, so the worker would keep serving a byte-for-byte
 * stale module from a previous dev session. That's what a "Failed to fetch
 * dynamically imported module" error on first navigation to a route usually
 * is in dev: the cached module's own nested imports no longer resolve
 * against the currently running dev server. In dev we instead actively tear
 * down any worker + cache a previous (pre-fix) dev session left behind.
 */
